...
class _LinearProgressIndicatorPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final double? effectiveValue = value == null
        ? null
        : clampDouble(value!, 0.0, 1.0);

    // Determinate progress indicator.
    if (effectiveValue != null) {
      final double trackStartFraction = trackGapFraction > 0
          ? effectiveValue +
                getEffectiveTrackGapFraction(effectiveValue, trackGapFraction)
          : 0;
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (trackColor != null) {
      ...
      if (hasGap && value != null && value! > _epsilon) {
        ...
        final double gapRadius = trackGap! / arcRadius;
        final double startGap = strokeRadius + gapRadius;
        final double endGap = startGap * 2;
