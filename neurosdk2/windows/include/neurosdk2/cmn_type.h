...
#define NEURO_EEG_MAX_CH_COUNT 32
...
typedef struct _Headphones2AmplifierParam {
	...
	enum SensorGain ChGain1;
	enum SensorGain ChGain2;
	enum SensorGain ChGain3;
	enum SensorGain ChGain4;

	enum GenCurrent Current;
...
{
	enum EEGChannelId Id;
	enum EEGChannelType ChType;
...
typedef struct _NeuroEEGAmplifierParam {
	...
	enum SensorSamplingFrequency Frequency;
	enum EEGRefMode ReferentMode;
	enum EEGChannelMode ChannelMode[NEURO_EEG_MAX_CH_COUNT];
	enum SensorGain ChannelGain[NEURO_EEG_MAX_CH_COUNT];
	uint8_t UseDiffAsRespiration;
...
typedef struct _NeuroEEGFSStatus {
	enum SensorFSStatus Status;
	enum SensorFSIOStatus IOStatus;
	enum SensorFSStreamStatus StreamStatus;
...
typedef struct _SensorBatteryGauge {
	double Level;
	double Current;
	double Voltage;
	double Temperature;
...
typedef void* SensorBatteryGaugeListenerHandle;
...
typedef	struct _SmartBandAmplifierParam {
	...
	enum SensorGain ChGain[SMART_BAND_MAX_CH_COUNT];
	enum GenCurrent Current;
...
typedef	struct _BrainBit2AmplifierParam {
	enum BrainBit2ChannelMode ChSignalMode[BRAINBIT2_MAX_CH_COUNT];
	uint8_t ChResistUse[BRAINBIT2_MAX_CH_COUNT];
	enum SensorGain ChGain[BRAINBIT2_MAX_CH_COUNT];
	enum GenCurrent Current;
...
enum SensorStimulType : uint8_t
{
	StimulTypeAcoustic = 0,
	StimulTypePhoto = 1
};

typedef void* NeuroStimulSyncStateListenerHandle;
