...
#include "cmn_type.h"
	...
	SDK_SHARED uint8_t readActiveChannelsNeuroEEG(Sensor* ptr, EEGChannelInfo* channelsOut, int32_t* szChannelsInOut, OpStatus* outStatus);
	...
	SDK_SHARED Sensor* readModuleStimNeuroEEG(Sensor* ptr);
	SDK_SHARED uint8_t writeModuleStimNeuroEEG(Sensor* ptr, Sensor* ptrNeuroStim, OpStatus* outStatus);
	SDK_SHARED uint8_t readCalibrateSignalType(Sensor* ptr, SensorStimulType* st, OpStatus* outStatus);
	SDK_SHARED uint8_t writeCalibrateSignalType(Sensor* ptr, SensorStimulType st, OpStatus* outStatus);

	SDK_SHARED uint8_t readLedStateNeuroEEG(Sensor* ptr, double* minBrightnessPrcOut, double* maxBrightnessPrcOut, OpStatus* outStatus);
	SDK_SHARED uint8_t writeLedStateNeuroEEG(Sensor* ptr, double minBrightnessPrc, double maxBrightnessPrc, OpStatus* outStatus);

	SDK_SHARED uint8_t readBatteryGaugeState(Sensor* ptr, SensorBatteryGauge* batteryGaugeOut, OpStatus* outStatus);
	SDK_SHARED uint8_t addBatteryGaugeStateCallback(Sensor* ptr, void(*callback)(Sensor* ptr, SensorBatteryGauge batteryGauge, void* userData), SensorBatteryGaugeListenerHandle* handleOut, void* userData, OpStatus* outStatus);
	SDK_SHARED void removeBatteryGaugeStateCallback(SensorBatteryGaugeListenerHandle handle);
	...
	SDK_SHARED uint8_t readNeuroStimSyncState(Sensor* ptr, SensorStimulSyncState* state, OpStatus* outStatus);
	SDK_SHARED uint8_t readNeuroStimTimeDefer(Sensor* ptr, double* timeOut, OpStatus* outStatus);
	SDK_SHARED uint8_t writeNeuroStimTimeDefer(Sensor* ptr, double time, OpStatus* outStatus);

	SDK_SHARED uint8_t addNeuroStimSyncStateCallback(Sensor* ptr, void(*callback)(Sensor* ptr, SensorStimulSyncState state, void* userData), NeuroStimulSyncStateListenerHandle* handleOut, void* userData, OpStatus* outStatus);
	SDK_SHARED void removeNeuroStimSyncStateCallback(NeuroStimulSyncStateListenerHandle handle);
